...
// write_runtime_gated: MCP never owns or directly executes the write. The one
// current schedule tool submits to loopback Operator authority, which durably
// owns identity/replay/approval before delegating execution to Agent Runtime.
...
export const PERMISSIONS: Record<string, PermissionEntry> = {
  ...
  "chanter.autoposter_schedule_post": { toolName: "chanter.autoposter_schedule_post", level: "write_runtime_gated", description: "Submit one durable Operator mission for independent approval (replay-safe; MCP cannot approve or publish).", requiresAudit: true, requiresApproval: true, requiresDryRun: false, requiresSafeCommitGate: false, requiresOperatorGate: true },
};
